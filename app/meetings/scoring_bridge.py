...
Promoted here from `app/styleguide/` in Phase 5, unchanged in shape. The
scoring worker needs exactly this translation and writing it twice is how the
two quietly disagree, so the move was a move rather than a rewrite.

**This module does five jobs and should do one.** Alongside the translation
proper — `_result_row` and `round_payload`, which is all the worker wants — it
carries scoring orchestration, display wording, meeting aggregation, and the
read queries behind nav, results and profiles. Splitting it into `bridge`,
`display` and `queries` is scheduled for Phase 7, which rewrites the callers
anyway; doing it during Phase 5 would have risked a silent styleguide
...
Phase 5 this module let `ruleset` default, which resolves to `CURRENT_VERSION`
— harmless while v1 was the only version in play, and a silent rewrite of
history the first time the places gained/lost magnitudes are re-tuned after
Jeddah. `Round.scoring_ruleset_version` exists for exactly this and is now read.
"""
